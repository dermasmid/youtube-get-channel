...
def execute(url, file_name, log_silently, txt, csv, markdown, reverse_chronological, headless, scroll_pause_time, user_driver, cookie_consent, execution_type):
    ...
    def verify_writing_to_at_least_one_file():
        if txt is False and csv is False and markdown is False:
            ...
            if execution_type == 'module':
                print(module_message.not_writing_to_any_files_hint)
            else:
                print(script_message.not_writing_to_any_files_hint)
            sys.exit()
    ...
    def open_user_driver():
        ...
        if user_driver is None:
            if execution_type == 'module':
                print(module_message.running_default_driver +
                      '\n' + module_message.show_driver_options)
            else:
                print(script_message.running_default_driver +
        ...
        if user_driver not in supported_drivers:
            ...
            sys.exit()
        # NOTE the need to CALL the function returned by supported_drivers[key] since the dictionary value is a function REFERENCE (the function is not yet invoked)
    ...
    def configure_safaridriver():
        if user_os != 'macos':
            common_message.display_dependency_setup_instructions(
                'safari', user_os)
            sys.exit()
        if headless is True:
    ...
    def handle_opening_webdriver_exception(error_message):
        ...
        # could not download the correct Selenium driver based on the user's OS and specified driver
        except selenium.common.exceptions.WebDriverException as same_error_message_again:
            common_message.display_selenium_dependency_update_error(
                same_error_message_again)
            traceback.print_exc()
            show_user_how_to_set_up_selenium()
            common_message.display_unable_to_update_driver_automatically(
                user_driver)
            sys.exit()
    ...
    def run_scraper():
        with driver:
            ...
            try:
                ...
                common_message.display_selenium_unable_to_load_elements_error(
                    error_message)
                traceback.print_exc()
                sys.exit()
            ...
            with yield_logger(file_name) as logging_locations:
                ...
                log(f'Now scraping {url} using the {user_driver}driver:',
                    logging_locations)
                program.determine_action(
                    url, driver, scroll_pause_time, reverse_chronological, file_name, txt, csv, markdown, logging_locations)
                program_end = time.perf_counter()
                total_time = program_end - program_start
                log(f'This program took {total_time} seconds to complete.',
                    logging_locations)
    ...
    def manage_cookie_consent_form():
        if 'consent.youtube.com' in driver.current_url:
            ...
            if cookie_consent is False:
                ...
                wait.until(EC.element_to_be_clickable(
                    (By.XPATH, '//button[@aria-label="Turn off Ad personalization"]')))    # last form element on page
                driver.find_element_by_xpath(
                ...
                # wait for last form element on page to update
                wait.until(EC.element_to_be_clickable(
                ...
                # find the last button on the page (the CONFIRM button) and click it
                ...
                common_message.display_invalid_cookie_consent_option(
                    cookie_consent)
    ...
    def yield_logger(file_name):
        ...
        with open(log_file, 'a', encoding='utf-8') as output_location:
            if log_silently is True:
                yield (output_location,)
            else:
                yield (output_location, sys.stdout)
